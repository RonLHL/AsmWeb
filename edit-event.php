    ...
    <body>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "GET") {
            ...
            if (empty($error)) {
                ...
                $sql = "UPDATE EVENT SET EVENT_NAME = ?, EVENT_ORGANIZER = ?, EVENT_IMG = ?, EVENT_DESC = ?, EVENT_VENUE = ?, STATUS = ?, DATE = ?, TIME = ?, PRICE = ? WHERE EVENT_ID = ?";

                $stmt = $con->prepare($sql);

                $stmt->bind_param("ssssssssds", $name, $organizer, $img, $desc, $venue, $status, $date, $time, $price, $id);
                $stmt->execute();
